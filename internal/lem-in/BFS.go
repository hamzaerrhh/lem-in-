...
func FindAllPaths(graph *types.GraphType) error {
	...
	paths := extractPaths(flowGraph, graph.Start, graph.End, roomIndex, indexRoom)
...
func extractPaths(fg *FlowGraph, start, end *types.Room, roomIndex map[*types.Room]int, indexRoom map[int]*types.Room) [][]*types.Room {
	paths := [][]*types.Room{}
